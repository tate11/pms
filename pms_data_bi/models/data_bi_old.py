...
class Data_Bi(models.Model):
    ...
    def export_data_bi(
        ...
        limit_ago = (
            fechafoto - timedelta(days=self.env.user.company_id.data_bi_days)
        ).strftime("%Y-%m-%d")
        ...
        if (archivo == 0) or (archivo == 8):
            dic_tipo_habitacion = self.data_bi_habitacione(compan.id_hotel, room_types)
    ...
    def data_bi_rooms(self, compan):
        ...
        for room in rooms:
            dic_rooms.append(
                {"ID_Hotel": compan, "ID_Room": room["id"], "Descripcion": room["name"]}
            )
        return dic_rooms
    ...
    def data_bi_canal(self, compan):
        ...
        for i in range(0, len(canal_array)):
            dic_canal.append(
                {"ID_Hotel": compan, "ID_Canal": i, "Descripcion": canal_array[i]}
            )
        return dic_canal
    ...
    def data_bi_hotel(self, compan):
        ...
        dic_hotel.append(
            {"ID_Hotel": compan.id_hotel, "Descripcion": compan.property_name}
        )
        return dic_hotel
    ...
    def data_bi_pais(self, compan):
        ...
        dic_pais.append(
            {"ID_Hotel": compan, "ID_Pais": "NONE", "Descripcion": "No Asignado"}
        )
    ...
    def data_bi_regimen(self, compan):
        ...
        dic_regimen.append(
            {"ID_Hotel": compan, "ID_Regimen": 0, "Descripcion": "Sin régimen"}
        )
    ...
    def data_bi_capacidad(self, compan, rooms):
        ...
        for room in rooms:
            dic_capacidad.append(
                {
                    "ID_Hotel": compan,
                    "Hasta_Fecha": (date.today() + timedelta(days=365 * 3)).strftime(
                        "%Y-%m-%d"
                    ),
                    "ID_Tipo_Habitacion": room["id"],
    ...
    def data_bi_client(self, compan):
        ...
        dic_clientes.append(
            {"ID_Hotel": compan, "ID_Cliente": 0, "Descripcion": u"Ninguno"}
        )
        ...
        for linea in lineas:
            dic_clientes.append(
                {"ID_Hotel": compan, "ID_Cliente": linea.id, "Descripcion": linea.name}
            )

        lineas = self.env["res.partner"].search([("is_tour_operator", "=", True)])
        id_cli_count = 700
        ...
        dic_clientes.append(
            {"ID_Hotel": compan, "ID_Cliente": 999, "Descripcion": u"Web Propia"}
        )
        dic_clientes.append(
            {
                "ID_Hotel": compan,
        ...
        dic_clientes.append(
            {"ID_Hotel": compan, "ID_Cliente": 903, "Descripcion": u"Puerta"}
        )
        dic_clientes.append(
            {"ID_Hotel": compan, "ID_Cliente": 904, "Descripcion": u"E-Mail"}
        )
        dic_clientes.append(
            {"ID_Hotel": compan, "ID_Cliente": 905, "Descripcion": u"Teléfono"}
        )
        dic_clientes.append(
            {"ID_Hotel": compan, "ID_Cliente": 906, "Descripcion": u"Call Center"}
        )
        dic_clientes.append(
            {"ID_Hotel": compan, "ID_Cliente": 907, "Descripcion": u"Agencia"}
        )
        dic_clientes.append(
            {"ID_Hotel": compan, "ID_Cliente": 908, "Descripcion": u"Touroperador"}
        )
        dic_clientes.append(
            {"ID_Hotel": compan, "ID_Cliente": 909, "Descripcion": u"Virtual Door"}
        )
        dic_clientes.append(
            {"ID_Hotel": compan, "ID_Cliente": 910, "Descripcion": u"Desvío"}
        )
        return dic_clientes
            ...
            dic_bloqueos.append(
                {
                    ...
                    "Fecha_hasta": (
                        datetime.strptime(line.date, "%Y-%m-%d") + timedelta(days=1)
                    ).strftime("%Y-%m-%d"),
    ...
    def data_bi_reservas(self, compan, lines, estado_array, dic_clientes):
        ...
        lineas = lines.filtered(
            lambda n: (n.reservation_id.reservation_type == "normal") and (n.price > 0)
        )
        ...
        for linea in lineas:
            ...
            if linea.reservation_id.board_service_room_id.id:
                regimen = (
                    linea.reservation_id.board_service_room_id.hotel_board_service_id.id
                )
            ...
            dic_reservas.append(
                {
                    ...
                    "ID_Hotel": compan,
                    "ID_EstadoReserva": estado_array.index(linea.reservation_id.state),
                    ...
                    "Entrada": linea.date,
                    "Salida": (
                        datetime.strptime(linea.date, "%Y-%m-%d") + timedelta(days=1)
                    ).strftime("%Y-%m-%d"),
                    "Noches": 1,
                    "ID_TipoHabitacion": linea.reservation_id.room_type_id.id,
                    "ID_HabitacionDuerme": linea.reservation_id.room_id.room_type_id.id,
                    "ID_Regimen": regimen,
    ...
    def data_bi_channel_cli(self, reserva, dic_clientes):
        ...
        if reserva.reservation_id.channel_type == "door":
            ...
            if tour.name:
                mach = next(
                    (l for l in dic_clientes if l["Descripcion"] == tour.name), False
                )
    ...
    def data_bi_comisiones_ota(self, reserva):
        ...
        if reserva.reservation_id.ota_id.ota_id == "2":
            ...
            if expedia_rate[3] == "MERCHANT":
                # EXPEDIA COLECT
                precio_comision = (
                    inv_percent(precio_neto, expedia_rate[1]) - precio_neto
                )
                ...
                if expedia_rate[2] != "NONE":
                    ...
                    if (fence_mes >= 4) and (fence_mes <= 10):
                        ...
                        if (fence_dia > 15) and (fence_mes == 10):
                            fence_dto = 7
                    precio_dto += (
                        inv_percent(precio_calculo, fence_dto) - precio_calculo
                    )
        ...
        response_dic.append(
            {
                "ota": reserva.reservation_id.ota_id.id,
    ...
    def data_bi_rate_expedia(self, reserva):
        if datetime.strptime(
            reserva.reservation_id.folio_id.date_order[:10], "%Y-%m-%d"
        ) < datetime(2019, 5, 9):
            comi_rate = 18
        else:
        ...
        if reserva.reservation_id.channel_bind_ids.channel_raw_data:
            data = json.loads(reserva.reservation_id.channel_bind_ids.channel_raw_data)
            ...
            if data.get("ancillary") is not None:
                ...
                if jsonBooked.get("ancillary").get("channel_rate_name") is not None:
                    json_rate = (
                        jsonBooked.get("ancillary").get("channel_rate_name").upper()
                    )
                    ...
                    data.get("booked_rooms")[0]
                    .get("roomdays")[0]
                    .get("ancillary")
                    .get("channel_rate_name")
                    is not None
                ):
                    json_rate = (
                        data.get("booked_rooms")[0]
                    ...
                    _logger.critical(
                        "EXPEDIA Tarifa No Contemplada: %s",
                        reserva.reservation_id.folio_id.name,
                    )
                    json_rate = "ROOM ONLY"
        else:
            _logger.error(
                "EXPEDIA NO RAW DATA: %s", reserva.reservation_id.folio_id.name
            )
            json_rate = "ROOM ONLY"
